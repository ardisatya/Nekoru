...
**Status:** Proposed untuk local/internal bootstrap. Tidak ada approval manusia yang diklaim.
...
Semua keputusan berikut adalah bootstrap local/internal yang dapat dibalik dan masih menunggu approver manusia.

| ID           | Keputusan         | Exact selection                                                                                                    | Status                    | Approval needed                    |
| ------------ | ----------------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------- | ---------------------------------- |
| `DEC-P0-001` | Package/workspace | pnpm `12.4.1`, Turborepo `2.10.12`, ESM, TypeScript strict                                                         | `proposed`                | Engineering Lead                   |
| `DEC-P0-002` | Web               | Next.js `16.3.5`, React `19.3.0`, App Router, CSS Modules                                                          | `proposed`                | Engineering + Design/Accessibility |
| `DEC-P0-003` | API/worker        | NestJS `11.2.3`, Fastify `5.12.4`, root override untuk transitive security patch, worker composition root terpisah | `proposed`                | Engineering + Security             |
| `DEC-P0-004` | Data              | PostgreSQL `18`, Drizzle ORM `0.45.2`, Drizzle Kit `0.31.10`                                                       | `proposed`                | Engineering + Data                 |
| `DEC-P0-005` | Canonical JSON    | `canonicalize@5.0.0` di belakang wrapper RFC 8785/JCS                                                              | `proposed`                | Engineering + Data                 |
| `DEC-P0-006` | Identity          | `@clerk/nextjs@7.9.2` dan `@clerk/backend@3.17.2` dengan local identity double; live config tidak diisi            | `pending_external_config` | Security/Privacy + Identity owner  |
| `DEC-P0-007` | Environment       | local/CI terpisah; target nonproduction `sin1`; tidak ada deployment yang dilakukan                                | `proposed`                | Operations + Security              |
...
| --------- | ----- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- |
| DEP-P0-01 | P0    | local_evidence_ready | Node 24 tersedia; migration dan tiga integration test lulus pada PostgreSQL 18.6 lokal ephemeral; tidak diterapkan ke shared environment | Technical/Data review + environment owner   |
| DEP-P0-02 | P0    | proposed             | Bootstrap package versions dipin di lockfile                                                                                             | Architecture owner                          |
...
| --------- | ----------------------------------------------------- | ------------------- | ------------------------- | ----------------------------------------------------------- |
| `DEP-001` | Domain/persistence model `0.0.0-phase0`               | `pending_review`    | Engineering + Product     | Engineering/Product review                                  |
| `DEP-002` | Zod + JSON Schema bundle `schema_version: 1`          | `pending_review`    | Engineering               | Engineering + domain owners                                 |
...
| `DEP-006` | Event envelope/catalog `schema_version: 1`            | `pending_review`    | Engineering + Data        | Payload/dedupe/privacy review                               |
...
| `DEP-010` | Telemetry allowlist/redaction contract `0.0.0-phase0` | `pending_review`    | Data + Security + Product | Event/property/sampling/success-metric approval             |
...
| -------------------- | -------------------------- | ------------------------------------------------------- | --------------------------- |
| Bootstrap decisions  | Engineering                | Architecture/Technical owner                            | `proposed`                  |
| Executable contracts | Engineering                | Technical/Data + domain owners                          | `pending_review`            |
